...
/**
 * 計算樹木生態效益
 * @param {object} tree  API 回傳的樹木物件（需含 species_name, dbh_cm, height_m, crown_m）
 * @returns {{ co2_kg: number, rain_L: number, airpoll_ntd: number } | null}
 */
function calcBenefits(tree) {
  const D = parseFloat(tree.dbh_cm);
  const H = parseFloat(tree.height_m);
  if (!D || !H) return null;
  ...
  return { co2_kg, rain_L, airpoll_ntd };
}
