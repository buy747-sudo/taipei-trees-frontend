...
- **2026-06-12 更新**：風險評估管理模式升級，作為登入後的專業作業功能，不開放一般民眾：
  - 首頁登入後 `inspector`、`contractor_admin`、`platform_admin`、政府角色可看到「評估紀錄」入口。
  - `risk.html` 顯示評估人員本週完成數，並鼓勵優先處理高風險與傾斜超過 30 度的樹木。
  - `risk-list.html` 新增本週進度面板與進階篩選：關鍵字、日期區間、行政區、樹種、評估等級、評估人員 ID、傾斜超過 30 度。
  - 後端 `/api/assessment/list` 支援公司/政府權限範圍與上述篩選；`/api/assessment/weekly-summary` 回傳個人與團隊本週統計。
  - 權限決策：同公司員工可看同公司紀錄；公司主管與政府/最高管理者可用同一套列表管理大量紀錄；一般民眾不可見。
  - 傾斜超過 30 度目前用風險題目 `q7` 的重大扣分答案判斷，後續若後端有實測角度欄位再改為數值判斷。

- **2026-06-12 更新**：風險評估管理頁主管看板補強：
  - `risk-list.html` 本週進度改為工作看板：個人完成量、A/B 級或傾斜 > 30 度數量、團隊排行。
  - 新增「每人目標」前端設定（localStorage，預設 20 棵/週），以進度條顯示完成率；這是展示與管理輔助，不寫入後端。
  - 評估人員篩選由手打 ID 改成依 `/weekly-summary` 團隊名單產生下拉選單；主管可在團隊列點「查看」直接套用該人員篩選。
  - tree-app `test_platform_auth.py` 紅燈已修正並由 GitHub Actions 驗證 success；原因是前台 `platform_users` 與內部 `admins` 帳號分離後，測試 fixture 必須跟著改。
