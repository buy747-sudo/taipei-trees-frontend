...
### RTK GPS 採集整合規劃（2026-06-07 新增）

- 評估 **Emlid Reach RX** 整合進 taipei-trees.org 普查作業的可行性。
- 建議架構：Reach RX → ReachView 3（Mock Location）→ `navigator.geolocation` → PWA，不需額外 BLE 程式碼。
- 已研究 Emlid 官網：Reach RX 規格為 **Bluetooth 4.2（BR/EDR + LE）**，售價 **$1,599 USD**（約 51,000 NTD）。
- 已寄送技術詢問信至 `support@emlid.com`，詢問三個關鍵問題：
  1. NMEA 輸出走 BR/EDR 還是 BLE？（決定能否用 Web Bluetooth 直接讀）
  2. Emlid Flow Android 是否支援 Mock Location Provider？
  3. iOS 第三方 web app 的 RTK 座標整合路徑為何？
- 待 Emlid 回覆後，決定最終整合架構。
- 規劃功能（待確認後實作）：
  - `gps-survey.html` 精度指示燈（RTK/一般 GPS）+ 批次採集模式
  - IndexedDB 離線暫存 + 有網路自動同步
  - `/api/survey/export-shp` Shapefile 匯出（tree-app 後端新增）
